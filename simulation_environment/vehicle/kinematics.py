...
class Vehicle(RoadObject):
    ...
    def set_predict_trajectory(self, past_traj: np.ndarray, future_traj: np.ndarray):
        if not isinstance(past_traj, np.ndarray) or not isinstance(future_traj, np.ndarray):
            raise ValueError("Trajectories must be numpy arrays")
        self.past_traj = past_traj
        self.future_traj = future_traj


class TrajTrackingVehicle(Vehicle):
    def on_state_update(self) -> None:
        if self.road and not self.name.startswith('ped'):
            self.lane_index = self.road.network.get_closest_lane_index(self.position, self.heading)
            self.lane = self.road.network.get_lane(self.lane_index)
