...
from simulation_environment.road.road import LaneIndex, Road, Route
...
class ControlledVehicle(Vehicle):
    ...
    def act(self, action: Union[dict, str] = None) -> None:
        ...
        if action == "FASTER":
            ...
            self.target_lane_index = self.target_lane_index if right_lane is None else (right_lane[0], right_lane[1], 0)
        elif action == "LANE_LEFT":
            left_lane = self.road.network.get_left_lane(self.lane_index)
            self.target_lane_index = self.target_lane_index if left_lane is None else (left_lane[0], left_lane[1], 0)
        elif action == "LEFT_1":
            ...
            self.pos_offset = -(w / 2 - 0.2 - lat)
        elif action == "RIGHT_2":
            long, lat = self.lane.local_coordinates(self.position)
            w = self.lane.width_at(long)
            self.pos_offset = lat + w / 2 - 0.2

        action = {"steering": self.steering_control(self.target_lane_index),
                  "acceleration": self.speed_control(self.target_speed)}
        action['steering'] = np.clip(action['steering'], -self.MAX_STEERING_ANGLE, self.MAX_STEERING_ANGLE)
        action['acceleration'] = np.clip(action['acceleration'], -3, 3.5)
        super().act(action)
