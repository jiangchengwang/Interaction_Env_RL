...
# from simulation_environment.common.finite_mdp import finite_mdp
from simulation_environment.osm_envs.env_graphics import EnvViewer
...
class AbstractEnv(gym.Env):
    ...
    def __init__(self, osm_path=None, config: dict = None, render_mode: Optional[str] = None) -> None:
        super().__init__()
    ...
    def load_map(self):
        if self.osm_path is None:
            # print("The osm_path must be initialized in the environment implementation")
            return

        projector = lanelet2.projection.UtmProjector(lanelet2.io.Origin(0.0, 0.0))
        self.laneletmap = lanelet2.io.load(self.osm_path, projector)
        self.roads_dict = utils.load_lanelet_map(self.laneletmap)
