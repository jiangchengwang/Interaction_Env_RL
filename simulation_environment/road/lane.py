...
from simulation_environment.road.spline import LinearSpline2D
from simulation_environment.utils import class_from_path, get_class_path, wrap_to_pi
...
class AbstractLane(object):
    ...
    @abstractmethod
    def local_coordinates(self, position: np.ndarray, max_lat: float = 5.0) -> Tuple[float, float]:
        """
...
class PolyLaneFixedWidth(AbstractLane):
    ...
    def __init__(
        ...
        lane_index: Tuple[int, int]=None,
        lane_id: str = None,
        reverse_lane_id: str = None,
        reverse_lane_spline=None,
    ) -> None:
        self.curve = LinearSpline2D(lane_points)
        self.spline = Spline2D(np.array(lane_points)[:, 0], np.array(lane_points)[:, 1])
        ...
        self.reverse_lane_id = reverse_lane_id
        self.reverse_lane_spline = reverse_lane_spline
    ...
    def local_coordinates(self, position: np.ndarray, max_lat=5.0) -> Tuple[float, float]:
        lon, lat = self.curve.cartesian_to_frenet(position, max_lat=max_lat)
        return lon, lat

    def cartesian_to_frenet2D(self, position: np.ndarray, heading: float, speed: float) -> Tuple[float, float, float, float]:
        """Convert a world position to local lane coordinates."""
        s, r = self.spline.cartesian_to_frenet1D(position[0], position[1])
        angle = self.local_angle(heading, s)
        s_d, r_d = speed * np.cos(angle), speed * np.sin(angle)
        return s, r, s_d, r_d
...
class PolyLane(PolyLaneFixedWidth):
    ...
    def __init__(
        ...
        super().__init__(
            ...
            reverse_lane_spline=reverse_lane_spline,
        )
