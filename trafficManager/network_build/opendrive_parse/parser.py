...
from trafficManager.network_build.opendrive_parse.elements.opendrive import OpenDrive, Header
from trafficManager.network_build.opendrive_parse.elements.road import Road
from trafficManager.network_build.opendrive_parse.elements.roadLink import (
    Predecessor as RoadLinkPredecessor,
    Successor as RoadLinkSuccessor,
    Neighbor as RoadLinkNeighbor,
)
from trafficManager.network_build.opendrive_parse.elements.roadtype import (
    RoadType,
    Speed as RoadTypeSpeed,
)
from trafficManager.network_build.opendrive_parse.elements.roadElevationProfile import (
    ElevationRecord as RoadElevationProfile,
)
from trafficManager.network_build.opendrive_parse.elements.roadLateralProfile import (
...
from trafficManager.network_build.opendrive_parse.elements.roadLanes import (
...
from trafficManager.network_build.opendrive_parse.elements.junction import (
    Junction,
...
def parse_opendrive(root_node) -> OpenDrive:
    ...
    # header = root_node.find(".//od:header", namespaces={'od': 'http://www.opendrive.org'})
    if header is not None:
        parse_opendrive_header(opendrive, header)

    # Junctions
    for junction in root_node.findall("junction"):
    # for junction in root_node.findall(".//od:junction", namespaces={'od': 'http://www.opendrive.org'}):
        parse_opendrive_junction(opendrive, junction)

    # Load roads
    for road in root_node.findall("road"):
    # for road in root_node.findall(".//od:road", namespaces={'od': 'http://www.opendrive.org'}):
