...
import abc
import numpy as np
from trafficManager.network_build.opendrive_parse.elements.eulerspiral import EulerSpiral
...
class Arc(Geometry):
    ...
    def calc_position(self, s_pos):
        ...
        # delta_x = np.cos(hdg) * s_pos
        # delta_y = np.sin(hdg) * s_pos
        #
        # # 更新朝向
        # hdg += c * s_pos
        #
        # pos = self.start_position + np.array([delta_x, delta_y])
        # tangent = self.heading + s_pos * self.curvature
